...
export class ContactPage {
    ...
    if(this.items){
      this.favorites.get().then((val) => {
        ...
        this.items.map(search_obj => {
          ...
          this.favoriteItems.map(obj => { console.log('favorite', obj); search_obj.Title === obj.Title ? search_obj.favorited = true : search_obj.favorited = false; })
        })
      });
  ...
  searchMovies(title, year){
    this.favorites.get().then((val) => {
      if(val){
        this.favoriteItems = val;
        this.api.getMovies(title, year, 'series')
          .subscribe(data => {
            if(!data.Search){
              this.presentToast();
            } else{
              data.Search.map(search_obj => {
                this.favoriteItems.map(obj => { search_obj.Title === obj.Title ? search_obj.favorited = true : search_obj.favorited = false; })
              })
